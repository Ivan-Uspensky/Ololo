	...
	void OnSceneGUI ()
	{
		...
		for ( int i = 0; i < m_Graph.nodes.Count; i++ )
		{
			...
			for ( int j = 0; j < node.connections.Count; j++ )
			{
				...
				Vector3 from = new Vector3(node.transform.position.x, node.transform.position.y, node.transform.position.z);
        Vector3 to = new Vector3(connection.transform.position.x, connection.transform.position.y, connection.transform.position.z);
